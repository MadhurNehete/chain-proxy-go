...
func TestTransactionProcessor_SendMultipleTransactionsPartialObserverFailure(t *testing.T) {
	t.Parallel()
	...
	p, _ := process.NewTransactionProcessor(
		&mock.ProcessorStub{
			ComputeShardIdCalled: func(addressBuff []byte) (uint32, error) {
				return 0, nil
			},
			GetObserversCalled: func(shardID uint32, dataAvailability data.ObserverDataAvailabilityType) (observers []*data.NodeData, e error) {
				return nil, process.ErrMissingObserver
			},
		},
		&mock.PubKeyConverterMock{},
		hasher,
	...
	response, err := p.SendMultipleTransactions(txsToSend)
	require.Error(t, err)
	require.Contains(t, err.Error(), "partial send failure")
	require.Equal(t, uint64(0), response.NumOfTxs)
	require.Empty(t, response.TxsHashes)
}
